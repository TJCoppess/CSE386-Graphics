...
void systematic(const dvec3& a, const dvec3& b, const dvec3& c, const ITriangle& tri) {
	...
	for (double z = mins.z; z <= maxs.z; z += DZ) {
		for (double y = mins.y; y <= maxs.y; y += DY) {
			...
			cout << i++ << ' ' << x << ' ' << y << ' ' << z << ' ' << msg(tri.inside(dvec3(x, y, z))) << endl;
		}
	}
}
...
Tri 2 - systematic
1 -4.996 4.98 -0.03 OUT
2 -5.496 7.48 -0.03 OUT
3 -5.996 9.98 -0.03 OUT
4 -4.996 4.98 2.47 OUT
5 -5.496 7.48 2.47 IN
6 -5.996 9.98 2.47 OUT
7 -4.996 4.98 4.97 OUT
8 -5.496 7.48 4.97 OUT
9 -5.996 9.98 4.97 OUT
